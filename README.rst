...
É possível negar/excluir linhas selecionadas utilizando o argumento ( ! ) :
...
Para identificar o número de campos/colunas ( NF ), delimitados por tabulação ( TAB ) ou barra de espaço, utiliza-se o comando:
...
No exemplo, será filtrado todas as linhas, da primeira coluna, que são exatamente iguais aos caracteres (DNA); Nesse caso, os caracteres são indicados entre as aspas duplas
...
5. Argumento ( != ): Esse argumento significa diferente, indicando que o campo/coluna é diferente do caractere ou valor filtrado
...
É possível criar ou substituir campos/colunas através de cálculos:
